...
export class SidePanelComponent implements OnInit {
		...
		var marginController = {
			leftMarginController: function (array) {
				...
				for (var counter = 0; counter < array.length; counter++) {
					...
					} else if (largestStringLength < 4) {
						largestStringLength = 3;
					}
	...
	/*	var minMaxController = {
		...
		getTickArray: function (minVal, maxVal, noOfTicksRequired) {
			var tickArray = [];
			var factor = ((maxVal - minVal) / (noOfTicksRequired - 1));
		...
		getXTickArray: function (minVal, maxVal, maxCharacterLength, svgWidth) {
			...
			var totalTicks = (svgWidth / maxTickWidth);
	...
	function getXscaleArray(arr) {
		var len = arr.length;
		var NewArr = [];
		for (var i = 0; i < len; i++) {
			var res = arr[i].split(":").map(function (i) {
				return parseInt(i, 10);
			});
			var temp = res[1] * 100 / 60 + (res[2] * 100 / 60) / 100;
			NewArr.push(res[0] + temp / 100);
		}
		return NewArr;
	}

	function getRealxscaleWidth(str) {
		var res = str.split(":").map(function (i) {
			return parseInt(i, 10);
		});
		var temp = res[1] * 100 / 60 + (res[2] * 100 / 60) / 100;
		return (res[0] + temp / 100);
	}

		previousCloseLineData = parseFloat(previousCloseLineData);
		todayDate = todayDate + " ";
		document.getElementById(`${this.marketSymbols[index]}-chart-container`).innerHTML = "";
		var data = {
			title: "Sold Quantity in Last 10 Month ",
			xAxisTickArray:chartXvalues,
		...
		var height = 120;
		var width = 237;
		var svgElement = d3.select(`#${this.marketSymbols[index]}-chart-container`)
			.append("svg")
			.attr("width", width)
			.attr("height", height);
		...
		var maxValue = 0;
		var minValue = 999999;
		var currMaxValue, currMinValue, strokeWidth = data.lineStrokeWidth;
		var actualData = data.yAxisData;
		var yAxisTickController = actualData[0].data;
		var leftMargin = 20;
		var dummyyScalePoints = [(previousCloseLineData - 0.01), (previousCloseLineData), (previousCloseLineData + 0.01)];

		if (yAxisTickController != undefined && yAxisTickController.length != 0) {
			leftMargin =25// marginController.leftMarginController(yAxisTickController);
		}
		else {
			for (var j = 0; j < dummyyScalePoints.length; j++) {
				dummyyScalePoints[j] = dummyyScalePoints[j].toFixed(2);
			}
			leftMargin = 25//marginController.leftMarginController(dummyyScalePoints);
		}
